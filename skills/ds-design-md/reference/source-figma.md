...
Use two layers together: a `ds-snapshot-figma` (or a live equivalent) for exact
token values, and rendered screenshots for the prose that describes how those
tokens combine.

## Layer 1 — tokens, text styles, components

**Prefer a `ds-snapshot-figma` over reading Figma live**, when one exists and is
recent:

- Check for `ds-inventory/snapshots/figma/<date>/` (or a `.bundle.json`) in the current
...
- If it's stale or missing and the user wants a one-off document rather than
  ...
  full `ds-snapshot-figma` skill. But if the user is going to want this file kept
  in sync over time, or wants the dependency layer (what components bind
  which tokens), point them at `ds-snapshot-figma` instead of duplicating its work
  here.

**Reading live**, when no snapshot applies:

- Confirm the Desktop Bridge is paired (`figma_get_status` with
  `probe: true`) before anything else, exactly as `ds-snapshot-figma` does. Don't
...
- `figma_get_variables` / `figma_export_tokens` for colour and spacing/radius
  ...
  exactly `ds-snapshot-figma`'s step 2, 4, and 5 — reuse its
...
- **A design system spanning several files** (a common Foundations /
  ...
  `figma_list_open_files` before starting, same as `ds-snapshot-figma` does.
...
Follow the same collection/mode/id logic `ds-snapshot-figma` uses if you're reading
